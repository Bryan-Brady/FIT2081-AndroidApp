...
public class MainActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    ...
    boolean topLeftCondition = false;
    boolean bottomRightCondition = false;
    ...
    class MyWeek11GestureHandlers extends GestureDetector.SimpleOnGestureListener{
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            taps++;
            Log.d("week_11", String.valueOf(taps));
            if (taps == 1){
                iniX = (int)e.getX();
                iniY = (int)e.getY();
                Log.d("week_11", "Initial Coords: (" + String.valueOf(iniX) + ", " + String.valueOf(iniY) + ")");
                if (iniX < 80 && iniY < 30){
                    topLeftCondition = true;
                }
                if (taps == 1 && topLeftCondition == false){
                    int incrementCost = getCost() + 150;
                    getCostView.setText(String.valueOf(incrementCost));
                }
            }else if (taps == 2){
                finalX = (int)e.getX();
                finalY = (int)e.getY();
                Log.d("week_11", "Final coords: (" + String.valueOf(finalX) + ", " + String.valueOf(finalY) + ")");
                if (finalX > 1000 && finalY > 140){
                    bottomRightCondition = true;
                }
                if (taps == 2 && bottomRightCondition == false){
                    int incrementCost = getCost() + 150;
                    getCostView.setText(String.valueOf(incrementCost));
                }
                if (topLeftCondition == true && bottomRightCondition == true){
                    String lowerCaseName = getMovieName().toLowerCase();
                    Log.d("week_11", "Title to lower case: " + lowerCaseName);
                    getMovieView.setText(lowerCaseName);
                }
                topLeftCondition = false;
                bottomRightCondition = false;
                taps = 0;
            }
        ...
        switch(id){
            ...
            case R.id.drawer_layout_list_all_movies_id:
                ...
                */
                ...
                Intent intent = new Intent (MainActivity.this,RecyclerActivity.class);
                intent.putExtra("getdbalist", dbAList);
                 ...
                 */
                break;
            case R.id.select_all_from_database_id:
